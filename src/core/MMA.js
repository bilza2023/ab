...
export class MMA {
  ...
  deposit({ supplierId, shade, size, qty, reason = 'DEPOSIT', meta }) {
  ...
  withdraw({ supplierId, shade, size, qty, reason = 'WITHDRAW', meta }) {
    this._require(supplierId, 'supplierId');
    this._require(shade, 'shade');
    this._requireQty(qty);
  ...
  dispatch({ toMmaCode, transportId, supplierId, shade, size, qty, meta }) {
